...
def main():
    try:
        ...
        if p_cnt > o_cnt:
            ...
            for i in range(middle_n):
                for j in range(0, middle_n):
                    if (i + j + 1) <= middle_n and i >= j:
                        C[i][j], C[(middle_n - i) - 1][(middle_n - j) - 1] = C[(middle_n - i) - 1][(middle_n - j) - 1], \
                            C[i][j]  # Симметрично меняем местами области 1 и 3 в "С"
            ...
            print(f'\nВ матрице "C" количество положительных элементов в четных столбцах в области 2({p_cnt})')
            print(
                f'меньше чем количество отрицательных элементов в нечетных столбцах в области 4({o_cnt}) или равно ей')
